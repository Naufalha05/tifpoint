...
export default function Competencies({ showSuccessNotification }) {
  ...
  // Base API URL for competencies
  const API_BASE_URL = 'https://tifpoint-production.up.railway.app/api/competencies';
    ...
    const fetchCompetencies = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const competenciesArray = Array.isArray(data) ? data : data.competencies || [];
          setCompetencies(competenciesArray);
          setFilteredCompetencies(competenciesArray);
        } else if (response.status === 401) {
          setError('Sesi login telah berakhir. Silakan login kembali.');
        } else if (response.status === 403) {
          setError('Anda tidak memiliki izin untuk mengakses data kompetensi');
        } else {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(errorData.error || 'Gagal memuat data kompetensi');
        }

        setLoading(false);
      } catch (err) {
        console.error('Error fetching competencies:', err);
        setError(err.message || 'Terjadi kesalahan saat memuat data kompetensi');
        setLoading(false);
      }
    };
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      errors.name = 'Nama kompetensi wajib diisi';
    } else if (formData.name.trim().length < 3) {
      errors.name = 'Nama kompetensi minimal 3 karakter';
    }

    if (!formData.description.trim()) {
      errors.description = 'Deskripsi kompetensi wajib diisi';
    } else if (formData.description.trim().length < 10) {
      errors.description = 'Deskripsi kompetensi minimal 10 karakter';
    }
  ...
  const openEditModal = (competency) => {
    ...
    if (!competency.id) {
      setError('ID kompetensi tidak valid.');
      return;
    }
    
    setItemToEdit(competency);
    setFormData({
  ...
  // Open delete modal
  const openDeleteModal = (competency) => {
    setError(null);
  ...
  const handleCreateCompetency = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFilteredCompetencies(prev => [...prev, newCompetency]);
        closeCreateModal();
        
        if (showSuccessNotification) {
          showSuccessNotification('Kompetensi berhasil ditambahkan!');
        }
        ...
        setError('Anda tidak memiliki izin untuk menambah kompetensi.');
      } else {
        const errorData = await response.json().catch(() => ({}));
        setError(errorData.error || 'Gagal menambah kompetensi');
      }
    } catch (err) {
      console.error('Error creating competency:', err);
      setError(err.message || 'Terjadi kesalahan saat menambah kompetensi');
    } finally {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCompetencies(prev => prev.map(comp => 
          comp.id === itemToEdit.id ? updatedCompetency : comp
        ));
        setFilteredCompetencies(prev => prev.map(comp => 
          comp.id === itemToEdit.id ? updatedCompetency : comp
        ));
        ...
        if (showSuccessNotification) {
          showSuccessNotification('Kompetensi berhasil diperbarui!');
        }
        ...
        setError('Anda tidak memiliki izin untuk mengubah kompetensi.');
      } else if (response.status === 404) {
        setError('Kompetensi tidak ditemukan.');
      } else {
        const errorData = await response.json().catch(() => ({}));
        setError(errorData.error || 'Gagal mengubah kompetensi');
      }
    } catch (err) {
      console.error('Error updating competency:', err);
      setError(err.message || 'Terjadi kesalahan saat mengubah kompetensi');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeleteCompetency = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCompetencies(prev => prev.filter(comp => comp.id !== itemToDelete.id));
        setFilteredCompetencies(prev => prev.filter(comp => comp.id !== itemToDelete.id));
        
        closeDeleteModal();
        
        if (showSuccessNotification) {
          showSuccessNotification('Kompetensi berhasil dihapus!');
        }
        ...
        setError('Anda tidak memiliki izin untuk menghapus kompetensi.');
      } else if (response.status === 404) {
        setError('Kompetensi tidak ditemukan.');
      } else {
        const errorData = await response.json().catch(() => ({}));
        setError(errorData.error || 'Gagal menghapus kompetensi');
      }
    } catch (err) {
      console.error('Error deleting competency:', err);
      setError(err.message || 'Terjadi kesalahan saat menghapus kompetensi');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="bg-white rounded-lg shadow transform transition-all duration-500 hover:shadow-lg">
      <div className="p-6 border-b border-gray-200">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold">Manajemen Kompetensi</h2>
        ...
        <div className="mt-4 relative">
          <input
            type="text"
            placeholder="Cari berdasarkan nama atau deskripsi kompetensi..."
      ...
      {loading ? (
        <div className="flex justify-center items-center p-8">
          ...
          <p className="ml-3 text-gray-600 animate-pulse">Memuat data kompetensi...</p>
        </div>
      ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredCompetencies.length > 0 ? (
                filteredCompetencies.map((competency, index) => (
                  <tr key={competency.id} className="hover:bg-gray-50 transition-colors duration-200 animate-slideInUp" style={{animationDelay: `${index * 100}ms`}}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{competency.name}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      <div className="max-w-xs">
                        <p className="truncate" title={competency.description}>
                ...
                <tr>
                  <td colSpan="3" className="px-6 py-4 text-center text-sm text-gray-500">
                    {searchQuery ? 'Tidak ada kompetensi yang sesuai dengan pencarian' : 'Tidak ada data kompetensi'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 transform transition-all duration-300 animate-slideInUp">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Tambah Kompetensi Baru</h3>
            </div>
      ...
      {isEditModalOpen && itemToEdit && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 transform transition-all duration-300 animate-slideInUp">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Edit Kompetensi</h3>
            </div>
            <div className="px-6 py-4">
              <div className="space-y-4">
                <div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 transform transition-all duration-300 animate-slideInUp">
            ...
            <div className="px-6 py-4">
              <p className="text-sm text-gray-600">
                Apakah Anda yakin ingin menghapus kompetensi <strong>"{itemToDelete.name}"</strong>?
              </p>
